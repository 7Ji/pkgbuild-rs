...
while read -r line; do
  ...
  for _items in  "${_pkgbuild_array_items[@]}"; do
    delcare -n _pkgbuild_items="${_items}"
  ...
  for _pkgname in "${pkgname[@]}"; do
    ...
    IFS="${_ifs_stored}"
    _buffer=
    for _line in "${_lines[@]:2:$((${#lines[@]}-3))}"; do 
      if [[ "${_buffer}" ]]; then
        _buffer+="
        ${_line}"
        if [[ "${_line}" == *');' ]]; then
      ...
      elif [[ "${_line}" =~ (pkgdesc|url|install|changelog)'="' ]] 
        if [[ "${_line}" == *');' ]]; then
          eval "${_line}"
        else
      ...
      elif [[ "${_line}" =~ (license|groups|backup|options)'=('* ]]; then
        if [[ "${_line}" == *');' ]]; then
          eval "${_line}"
        else
          buffer="${_line}"
  ...
  unset -v \
    pkgdesc url license groups backup options install changelog \
    package{,_"${pkgbase}"} "${pkgname[@]/#/package_}"
  echo END
